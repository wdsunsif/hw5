...
var debtorsWithMatchingEmailDomains = debtors.Where(d => d.Email.EndsWith("rhyta.com") || d.Email.EndsWith("dayrep.com"));

Console.WriteLine("Debtors with email domains rhyta.com or dayrep.com:");
foreach (var debtor in debtorsWithMatchingEmailDomains)
{
    Console.WriteLine(debtor);
}

var currentDate = DateTime.Now;
var debtorsBetweenAges = debtors.Where(d => (currentDate.Year - d.BirthDay.Year) >= 36 && (currentDate.Year - d.BirthDay.Year) <= 46);

Console.WriteLine("\nDebtors aged between 36 and 46:");
foreach (var debtor in debtorsBetweenAges)
{
    Console.WriteLine(debtor);
}

var debtorsWithDebtUnder5000 = debtors.Where(d => d.Debt <= 5000);

Console.WriteLine("\nDebtors with debt not exceeding 5000:");
foreach (var debtor in debtorsWithDebtUnder5000)
{
    Console.WriteLine(debtor);
}
